...
export class PptxExport extends Export {
  ...
  createTitleSlide(features: Feature[]): void {
    ...
    titleSlide.addText(
      [
        ...
        { text: `Data extracted on ${new Date().toISOString().slice(0, 10)}`, 
          options: { color: '333333', font_face: 'Georgia', font_size: 12 } }
      ],
    ...
    titleSlide.addText(
      ...
      { x: 0.44, y: 4.87, w: 5.72, h: 0.24, color: '333333', font_face: 'Georgia', font_size: 12 }
    );
  ...
  async createFeatureSlide(feature: Feature, index: number): Promise<void> {
    ...
    if (screenshot !== null) {
      featSlide.addImage({ data: screenshot, w: 8.84, h: 2.66, y: 0.36, x: 0.52 });
    } else {
      featSlide.addShape(this.pptx.shapes.RECTANGLE, { w: 8.84, h: 2.66, y: 0.36, x: 0.52, fill: 'eeeeee' });
    }
    ...
    featSlide.addText(
      ...
      { w: 9.15, h: 0.24, isTextBox: true, x: 0.44, y: 5.02, font_size: 11, font_face: 'Georgia', color: '333333' }
    );
  }
